...
/**
 * Protocole des requêtes possibles après la connexion au serveur, concernant
 * principalement la gestion de comptes.
 * 
...
public class AccountReceiveProtocol extends ServerStandardReceiveProtocol {
   ...
   public void joinLobby() {
      ...
      if (isFreeServer) {
         try {
            ...
            common.components.gameserver.PlayerStatus status = freeServer
                  .addPlayer(user);
            ...
            if (status != null) {
               ...
               GameServerConnection connection = new GameServerConnection(core,
                     freeServer, user, status);
               user.serverReceive = connection;

               // Confirmation d'avoir rejoint
               user.connectionsToClient.receiveChannel.sendBoolean(true);

               // Envoi du nombre de places dans le salon
               user.connectionsToClient.receiveChannel.sendInt(freeServer
               ...
               user.log.push("Error, there was a free lobby, but error while "
                           + "joining it.");
            }
   ...
   private void updateLogName() {
      if (user.account != null) {
         core.setLogPanelTitle(user.log.getLogPanel(), user.account.getLogin());
      }
      else {
         core.setLogPanelTitle(user.log.getLogPanel(), "Unknown");
      }
   }
}
